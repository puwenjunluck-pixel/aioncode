...
<!-- 本模板综合 AionCode 现有 plan 格式 + superpowers:writing-plans 的 bite-sized task 结构。
     See .aion/CREDITS.md
     使用方: aion-plan 命令 / aion-think 收敛后的主动建议流程,按此结构生成 `.aion/plans/{feature}.md`
-->
...
> **For agentic workers:** 用 superpowers:subagent-driven-development(推荐) 或
> superpowers:executing-plans 逐 task 执行。Steps 使用 `- [ ]` checkbox 跟踪。
> (若本项目不使用 superpowers,用 aion-implementing 流程或 /project:aion-loop 执行)
...
## Self-Review (aion-plan 写完后必做)

三维度检查,有问题**就地修**,不要回头重审:
...
## Execution Handoff (写完 plan 后)

保存到 `.aion/plans/{feature-name}.md` 后,向用户提供执行选项:

> Plan 已保存到 `.aion/plans/{filename}.md`。两种执行方式:
>
> **1. Subagent-Driven (推荐)** — 每个 task 派一个新 subagent,task 间 review,快速迭代
> **2. Inline Execution** — 本会话内逐 task 执行,checkpoint 分批 review
>
> 选哪个?

若用户选 Subagent-Driven:参考 `superpowers:subagent-driven-development` 或 AionCode 的 Agent 工具分发方式。
若用户选 Inline:用 `superpowers:executing-plans` 或 `/project:aion-loop` 。
