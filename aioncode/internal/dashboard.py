...
# Resolve source paths — adapt for package, PyInstaller, or standalone mode
SCRIPT_DIR = Path(__file__).resolve().parent
if getattr(sys, "frozen", False):
    # PyInstaller bundle
    _BUNDLE_DIR = Path(sys._MEIPASS)  # type: ignore[attr-defined]
    COMMANDS_SRC = _BUNDLE_DIR / "commands"
    TEMPLATES_SRC = _BUNDLE_DIR / "templates"
elif (SCRIPT_DIR / "templates").is_dir():
    # Package mode (internal/templates/ exists)
    COMMANDS_SRC = SCRIPT_DIR.parent.parent / "commands"
    TEMPLATES_SRC = SCRIPT_DIR / "templates"
else:
    # Standalone fallback (legacy: running from repo root)
...
def _resolve_projects_file() -> Path:
    ...
    env_path = os.environ.get("AIONCODE_PROJECTS_FILE")
    if env_path:
        return Path(env_path)
    # Package/frozen mode: use ~/.config/aioncode/ or %LOCALAPPDATA%/AionCode/
    if getattr(sys, "frozen", False) or "aioncode" in __name__:
    ...
    # Standalone: beside the script
    return SCRIPT_DIR / "projects.json"
...
def main(port: int = PORT, host: str = ""):
    global PORT
    PORT = port
    projects = load_projects()
