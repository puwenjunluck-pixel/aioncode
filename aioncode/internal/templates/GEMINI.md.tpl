...
Key rules:
- For tasks involving **3+ file changes**, ALWAYS run `/aion-think` first and get user approval (Phase 9 gate) before implementing. `aion-think` 完成后会**主动建议**进入 plan 阶段,**不需要用户显式触发** `/aion-plan`。
- `/aion-plan` 命令保留,仅用于**修改已有 plan**。主路径初次生成由 aion-think Phase 10 衔接。
- NEVER commit without running `/aion-review` first. commit requires review approval + Verification Gate 通过(Iron Law 2: evidence before claims)。
