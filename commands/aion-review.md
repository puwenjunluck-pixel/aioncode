...
When reviewing changes across 5+ files or across independent concerns, consider using the Agent tool to parallelize:
...
Review is split into two independent stages. Both read the COMPLETE file (not just diffs). When using subagents (Parallelism Strategy above), dispatch these as two parallel agents.
...
#### Merging Results
After both stages complete, merge findings into a single review. If stages were run by subagents, read both outputs and deduplicate before scoring.
