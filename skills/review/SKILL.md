---
name: review
description: 代码审查 + 学习飞轮。Use after ANY code change and ALWAYS before commit（门禁 hook 消费其报告）。Also 触发词：审查/review/审计/audit。Not for pure Q&A or 无改动场景。
---
...
**Arguments**: 可选指定文件或 "all"。`--quick`（跳过 test gap 分析）| `--deep`（全项目审计，见 `references/deep-mode.md`）| `--auto`（AUTO-FIX 类自动应用，ASK 类跳过并记录）。为空 = 审查全部未提交改动。
...
4. `--deep` → 读 `references/deep-mode.md` 并按其执行（全项目安全+性能审计，替代常规 diff-scope 流程）。注意：audit 报告不含 `reviewed_files` / `base_commit` 门禁字段，不解锁 commit 门禁 — 修复审计问题后仍需常规 `/aion:review`
...
打分前必须**在本 session** 跑验证命令：识别（什么命令证明代码真的能跑）→ 执行 → 读完整输出/exit code → 记入报告 `Verification` 段。「声明 → 需要的证据」对照表见宿主 `.claude/rules/metacognition.md` 的 Verification Before Claim。
**未跑验证 = 不能 approve**，verdict 降为 `needs_fix`。review 特有豁免出口：① 纯文档/配置变更 — 报告明确标 `Verification: N/A — pure doc/config` 并写明判断依据；② 绿地项目无任何测试基建 — 最小冒烟验证（运行入口文件 / 一条手写断言命令）可作为合法证据等级，报告中标注 `Verification: smoke-only` 并建议补测试基建。
...
先做机械扫描：若 `$CLAUDE_PLUGIN_ROOT` 可用，运行 `bash "$CLAUDE_PLUGIN_ROOT/scripts/rules-status.sh"`；不可用则按 last_cited 距今 >60 天（无论 cite_count）人工扫描。对每条 STALE 候选逐条向用户确认是否标 `status: archived`（NEVER 自动删除正文）。
...
写入 `.aion/reviews/{feature-name}.md`，完整模板见 `references/report-template.md`。**必填 frontmatter 字段（hook 机械消费，缺失 = 门禁拦截）**：
- `status: {approved | needs_fix}` — 唯一机读权威，hook 只读 status（正文 Verdict 仅展示用）
- `base_commit`: `git rev-parse --short HEAD` 的输出
- `reviewed_files`: repo 相对路径，与 `git diff --name-only` 输出一致，逐文件一行
...
收到审查反馈（来自用户、其他 agent 或本 skill 的产出）→ 读 `references/receiving-feedback.md` 执行（任一条不清晰即阻断全部条目 / 逐项实现逐项验证 / pushback 纠错姿势）。
高频红线常驻：**禁止 performative agreement** — "Great point!" / "You're absolutely right!" 一律不说；技术性回应或直接动手，actions > words。
...
- [ ] 规则提取走 write-protocol 去重；引用更新 + 60 天退役扫描执行（4c，机械扫描优先）
...
（Deep Mode 专属反模式与合理化条目见 `references/deep-mode.md`）

### Rationalization Prevention — 以下念头 = STOP，你在合理化
| 借口 | 真相 |
|--------|---------|
