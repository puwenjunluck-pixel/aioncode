...
Bug 修复:      /aion:fix(内含 fix(bug): 原子提交)→ /aion:review 事后审查

Key rules:
- 3+ 文件改动,ALWAYS 先跑 /aion:think 并获得用户批准(Phase 9 gate)再实现。think 完成后**主动衔接** plan,不需要显式触发 /aion:plan。
- NEVER commit without /aion:review first — 提交门禁 hook 机械校验 review 覆盖(Verification Gate 由 /aion:review 在 approve 前保证)。
...
| /aion:plan | 修改已有实现计划,或为已有 spec 补 plan |
