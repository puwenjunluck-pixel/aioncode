...
## Fix Laws（不可协商 — 见宿主 `.claude/rules/metacognition.md`，由 /aion:init 安装）
...
> 💡 **4-phase 根因分析（复现→根因→修复→回归验证）是默认路径** — 即使 bug 看起来简单（与 metacognition RULE 3「NO FIX WITHOUT ROOT CAUSE」同口径）。simple bug 有 simple 根因，走流程 2 分钟；跳过的代价是症状式修复 → bug 换个形式回归。只有 bug **极度明确**（已知 typo、已知空指针）且无疑义时才用 `--quick`，且仍须写出单一可检验假设。
...
> 映射说明：bug frontmatter `type: cross` ↔ 文件名前缀 `X-`（由 /aion:qa 分类产生）— 跨切 bug 对 frontend 与 backend 两个角色都相关，两个 scope 都纳入。
...
#### 2c.5 Root Cause Analysis（默认路径，4-phase）

`--quick` 时跳过 Phase 2 的完整根因对照，但**仍须写出单一可检验假设**「根因是 X 因为 Y」才能进入 Phase 3，且 summary 必须标注「快速模式：未做完整根因分析」。
...
#### 2e. Verify Test — 红→绿证据（Fix Law 3）
...
```
提交信息**必须以 `fix(bug): ` 开头**（hook 锚定校验；门禁官方豁免，见上文「提交与门禁的关系」）。One commit per bug，绝不 batch。口述模式无 BUG-ID 时用 `fix(bug): {模块} {一句话}`。`--auto`：自动 stage + commit，Step 3 汇总审计。
...
- [ ] 每个 bug：先定位精确代码再动手；Reuse Scan 做了；4-phase 根因分析默认完成（`--quick` 时仍写出单一可检验假设，summary 标注快速模式）
- [ ] 每个 bug：红→绿验证有证据（verify_test 或自构用例）；suspected 的 bug 修前已复现确认
- [ ] 每个 bug：原子提交（信息必须以 `fix(bug): ` 开头，hook 锚定校验）+ status 更新为 fixed
...
- `DONE_WITH_CONCERNS` — 批次结束存在跳过项（单 bug 2 次尝试失败 / suspected 复现失败 / 延期）
- `BLOCKED` — 角色不允许修 bug，或整批无法继续（环境/依赖等系统性阻塞）
